...
data2 = my_dataset(data=fake_data)
data2.y = torch.nn.functional.one_hot(data2.y) 
data2.y = data2.y.type(torch.float32)
...
train2 = [data2[i] for i in range(len(data2))]

join_train = train+train2

train_ld = DataLoader(join_train, batch_size=batch_size, shuffle=True)
